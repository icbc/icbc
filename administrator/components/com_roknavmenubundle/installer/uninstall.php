...
for ($i = 0; $i < count($cogs); $i++) {
	$cog =& $cogs[$i];
	if ($cog['id'] > 0 && _reset_core($db, $cog) && $cog['installer']->uninstall($cog['type'], $cog['id'], $cog['client_id'])) {
		$cog['status'] = true;
	}
	else {
		$err_status = true;
...
$remove_dir = JPath::clean(JPATH_THEMES.DS.$mainframe->getTemplate().'/html/com_menus');
JFolder::delete($remove_dir);
